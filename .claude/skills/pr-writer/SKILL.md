---
name: pr-writer
description: ALWAYS use this skill when creating or updating pull requests — never create or edit a PR directly without it. Enforces a consistent style for PR titles, descriptions, and issue references. Trigger on any create PR, open PR, submit PR, make PR, update PR title, update PR description, edit PR, push and create PR, prepare changes for review task, or request for a PR writer.
---

# PR Writer

Create pull requests with clear, consistent titles and descriptions.
...
If the output shows uncommitted changes that should be included, commit them first
(GPG-signed, per CLAUDE.md), then continue.
...
| Syntax          | Effect                            |
| --------------- | --------------------------------- |
| `Fixes #1234`   | Closes the GitHub issue on merge  |
| `Refs #1234`    | Links the issue without closing   |
...
**Preserve the CI-managed preview region.** This repo's `ci.yml` maintains a
`<!-- artifact -->…<!-- /artifact -->` block at the end of the PR body pointing at
the latest build. When you rewrite the body, keep that region intact (read the
current body first and re-append it) — CI will refresh its contents on the next run.
