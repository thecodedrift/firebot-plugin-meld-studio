...
- Use explicit return type annotations for exported/top-level functions
- Keep effects, events, filters, and variables in their own file, re-exported
  through the folder's `index.ts` (the pattern the rest of `src/` follows)
- Use proper error handling patterns (avoid try/catch unless it represents a true exception)
- Leave formatting to Prettier (`npm run format`); focus your edits on structure and clarity
