...
In this repo the required check is `Typecheck and build` (`.github/workflows/ci.yml`): `npx tsc --noEmit` then `npm run build`. When it fails, reproduce locally with those same commands before pushing a fix.
